...
var canvas = document.createElement("canvas"),
  ...
  starfield = document.getElementById("startrack");
...
var starContext = starfield.getContext("2d"),
  stars = [], // 用于存储星星对象的数组
  frameCount = 0; // 帧数计数器

// 生成两个值之间的随机数的函数
function randomBetween(t, e) {
  var range = e - t;
...
function updateFrame() {
  ...
  if (++frameCount > 150 && frameCount % 8 === 0) {
    ...
    starContext.fillRect(-3 * maxSize, -3 * maxSize, 6 * maxSize, 6 * maxSize);
  }
...
for (var i = 0; i < numStars; i++) {
  stars.push({
    ...
    size: 1.2,
    color: `rgba(${randomBetween(120, 255)},${randomBetween(120, 255)},${randomBetween(120, 255)},${randomBetween(30, 100) / 100})`,
  });
}
